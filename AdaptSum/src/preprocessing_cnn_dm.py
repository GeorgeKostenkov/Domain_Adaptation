...
def tokenizing_fn(instances, tokenizer):
    ...
    with tokenizer.as_target_tokenizer():
        labels = tokenizer(instances["highlights"], truncation=True,  add_special_tokens=False)
...
class BartDataset(Dataset):
    ...
    def __init__(self, tokenizer, source, target, data):
        ...
        temp = data.map(lambda x : tokenizing_fn(x, tokenizer), batched=True).remove_columns(['article', 'highlights'])
