...
class CorpusDataset(Dataset):
    def __init__(self, data, denoising_flag=False):
        self.data = []
    ...
    def train(self):
        ...
        for epoch_i in range(self.epoch):
            ...
            for sents in pbar:
                ...
                iteration += 1
                tokenized_sents = self.tokenizer(sents, add_special_tokens=False, truncation=True)['input_ids']
                ...
                # print("after:")
                # print(noisy_text[0])
                inputs_ids = self.tokenizer(noisy_text, add_special_tokens=False, truncation=True)['input_ids']
...
#                     pbar.set_description("(Epoch {}) LOSS: {:.6f} Euclid dist: {:.6f} LR: {:.6f}".format(epoch_i, np.mean(loss_list), dist_sum / dist_num, self.scheduler.get_last_lr()[0]))
                print("(Epoch {}) LOSS: {:.6f} Euclid dist: {:.6f}".format(epoch_i, np.mean(loss_list), dist_sum / dist_num))
            else:
                print("(Epoch {}) LOSS: {:.6f} LearningRate: {:.10f}".format(epoch_i, np.mean(loss_list), self.optimizer.learning_rate))
            if np.mean(loss_list) < best_loss:
                bad_epochs = 0
                best_loss = np.mean(loss_list)
                print('=====Saving checkpoint=====')
                print(epoch_i)
                model_name = args.save_path+'/{}_best.chkpt'.format(args.path, args.train_from)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-accum_step', type=int, default=1, help="accumulation steps")
    ...
    if (args.path == 'pubmed') or (args.path == 'arxiv'):
        data = load_dataset('scientific_papers', args.path, cache_dir='cache')['train'].train_test_split(train_size=args.train_size, shuffle=True, seed=42)['train']['article']
    else:
        data = load_dataset(args.path, cache_dir='cache')['train'].train_test_split(train_size=args.train_size, shuffle=True, seed=42)['train']['document']
    dataset = CorpusDataset(data)
