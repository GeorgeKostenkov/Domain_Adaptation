...
def tokenizing_fn(instances, tokenizer):
    encoded = tokenizer(instances["email_body"], truncation=True,  add_special_tokens=False)
    with tokenizer.as_target_tokenizer():
        labels = tokenizer(instances["subject_line"], truncation=True,  add_special_tokens=False)
...
class BartDataset(Dataset):
    ...
    def __init__(self, tokenizer, source, target, data):
        ...
        temp = data.map(lambda x : tokenizing_fn(x, tokenizer), batched=True).remove_columns(['email_body', 'subject_line'])
