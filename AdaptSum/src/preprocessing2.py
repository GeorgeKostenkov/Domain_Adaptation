...
def tokenizing_fn(instances, tokenizer):
    encoded = tokenizer(instances["article"], truncation=True, skip_special_tokens=True)
    with tokenizer.as_target_tokenizer():
        labels = tokenizer(instances["highlights"], truncation=True, skip_special_tokens=True)
...
#         print(self.src_ids)
#         print(data.map(lambda x : tokenizing_fn(x, tokenizer), batched=True).remove_columns(['article', 'highlights'])['input_ids'])
...
def data_builder(args):
    if args.data_name == 'ccdv/cnn_dailymail':
        data = load_dataset(args.data_name, '3.0.0', cache_dir='cache')[args.mode].train_test_split(train_size=20000, shuffle=True, seed=42)['train']
    else:
        data = load_dataset(args.data_name, cache_dir='cache')[args.mode].train_test_split(train_size=1, shuffle=True, seed=42)['train']
    ...
    if args.mode == 'train':
        ...
        data_loader = DataLoader(dataset=train_set,
                                ...
                                shuffle=False,
                                collate_fn=train_set.collate_fn)
#     save(data_loader, 'loaders/' + args.data_name)
